...
class GraphNet(object):
    ...
    def inference(self, outs):
        degree = tf.reduce_sum(self.matrix, 1, keep_dims=True)
        degree = tf.to_float(degree, name='ToFloat')
        degree = tf.nn.l2_normalize(degree)
        outs1 = tf.concat([outs, degree], axis=1, name='outdegree')
        outs = getattr(ops, self.conf.first_conv)(
            self.normed_matrix, outs1, 4 * self.conf.ch_num, self.conf.adj_keep_r,
            self.conf.keep_r, self.is_train, 'conv_s_1', act_fn=None)
        outs = getattr(ops, self.conf.first_conv)(
            self.normed_matrix, outs, 2 * self.conf.ch_num, self.conf.adj_keep_r,
            self.conf.keep_r, self.is_train, 'conv_s_1s', act_fn=None)
        for layer_index in range(1, self.conf.layer_num):
            cur_outs = getattr(ops, self.conf.first_conv)(
                self.normed_matrix, outs1, 4 * self.conf.ch_num, self.conf.adj_keep_r,
                self.conf.keep_r, self.is_train, 'conv_s_%s' % (layer_index+1), act_fn=None)
            cur_outs = getattr(ops, self.conf.first_conv)(
                self.normed_matrix, cur_outs, 2 * self.conf.ch_num, self.conf.adj_keep_r,
                self.conf.keep_r, self.is_train, 'conv_s_%ss'% (layer_index+1), act_fn=None)
    ...
    def process_data(self):
        if self.conf.dataset == 'citeseer':
            data = citeseer.CiteseerData().process_data()
        else:
            data = load_data(self.conf.dataset)
        adj, feas = data[:2]
    ...
    def train(self):
        ...
        return self.transductive_train()
    ...
    def transductive_train(self):
        ...
        for epoch_num in range(self.conf.max_step+1):
            ...
            if epoch_num and epoch_num % 100 == 0:
                self.save(epoch_num)
            print('step: %d --- loss: %.4f, train: %.3f, val: %.3f, test: %.3f' %(
                epoch_num, train_loss, train_accuracy, valid_accuracy,stats[2]))
            if stats[1] > 1000 and epoch_num > 1000:
